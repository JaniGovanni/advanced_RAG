...
def history_aware_query(chat_config: ChatConfig, query: str) -> str:
    ...
    """
    if chat_config.history_aware and chat_config.history_chain:
        processed_query = chat_config.history_chain.reformulate(input=query)
    else:
        processed_query = query
        # Append the query to the message history if not using history awareness
        chat_config.memory.chat_memory.messages.append(HumanMessage(content=query))
